...
- **PASKitLifecycle** — app-lifecycle UI: `presentAppRating`, `presentAppFeedback`, `FeedbackSheet`, `loading` overlay, `pasGlass`, `VersionCheckManager`, `AppUpdateView`, `WhatsNewView`, `ChangelogView`, `MailComposerView`, `AppInfoFooter`, onboarding engine (`PASOnboardingFlow`), `pasDevelopmentOverlay`, `pasToast`, `PASProgressRing`.
