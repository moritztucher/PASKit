...
//  ships `pasGlass(_:in:)` for surfaces (cards, sheet content, custom
//  backgrounds) and `pasGlassButtonStyle(_:)` for buttons. Apple's nav bar /
...
public extension View {
    ...
    func pasGlass(_ glass: PASGlass = .regular, in shape: some Shape = .rect) -> some View {
        modifier(PASGlassModifier(glass: glass, shape: shape))
    }
    ...
    @ViewBuilder
    func pasGlassButtonStyle(_ variant: PASGlassButtonVariant = .regular) -> some View {
...
private struct PASGlassModifier<S: Shape>: ViewModifier {
