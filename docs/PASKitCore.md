...
- `PASPressableButtonStyle` + `.buttonStyle(.pasPressable(…))` — the press-scale interaction (scale on press + spring, optional `PASHaptic` on the press-down edge) under apps' styled buttons. Brand-free — fills/fonts/radii stay per-app. Reduce Motion-aware (snap instead of spring). Extracted from two apps' scale-button styles.
